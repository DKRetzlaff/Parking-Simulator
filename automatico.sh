...
user="andreguipeil"                   #Descomentado no Automatico.sh para conter o nome do usuário do Github
project="Parking-Simulator"			    #Desbloqueado no Automatico.sh para conter o nome do projeto do Github
...
function criar_automatico()
{
	...
	if [ $enter = "Y" -o $enter = "y" ]; then
		...
		sed "s/project="Parking-Simulator"/project=\"$project\"/g" "temp1.sh" > "temp2.sh"
		echo "-    |--------------------%50                    |      -"
		#Coloca o valor do usuário no indentifador
		sed "s/user="andreguipeil"/user=\"$user\"/g" "temp2.sh" > "temp3.sh"
		echo "-    |--------------------------------%75        |      -"
